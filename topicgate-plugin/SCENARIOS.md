...
1. **Snapshot:** `inspect_broker(include_snapshot=true)`; report freshness, completeness, and topics; use `list_brokers` only for ambiguity; use legacy `get_broker_snapshot` only for filters or maximum age; never mutate.
...
10. **Live observation:** require control mode and explicit intent before `observe_broker_snapshot`; report broker activation, reconnection, waiting, and persistence.
